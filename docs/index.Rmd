...
The first visualization is going to be visualizing the total `box_office` by `year_release`. For the interactive portion, we're going to color each point by different categories.
...
The second visualization is going to be showing a boxplot of `box_office` by category:
...
1. Open the `01_app_basics` folder and open the `app.R` file. This is the 
